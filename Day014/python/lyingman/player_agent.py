...
class PlayerAgent:
    ...
    def _get_role_strategy(self, role: str) -> str:
        """获取角色专属策略提示"""
        strategies = {
            "预言家": """【预言家专属策略】
由于你的技能是报查验，所以身份可以公开。当你有查验结果时可以说"我昨晚查了XX，他是好人/狼人"。
策略：尽早跳明报查验，安排好警徽流，发言要有底气。""",

            "女巫": """【女巫专属策略】
你的身份是秘密的！绝对不要主动跳明说"我是女巫"。
策略：第一晚通常救人但也可以考虑自刀骗药，银水可能是狼人自刀，后期毒药追轮次。""",

            "猎人": """【猎人专属策略】
你的身份是秘密的！不要主动说"我是猎人"。
策略：保持强势发言给狼人压力，死亡时要仔细分析选择开枪目标。""",

            "守卫": """【守卫专属策略】
你的身份是秘密的！不要主动说"我是守卫"。
策略：第一晚通常守预言家，通过发言抿狼坑判断刀人方向。""",

            "白痴": """【白痴专属策略】
你的身份是秘密的！被投票出局可以翻牌免死。
策略：保持低调不要引起注意，如果被抗推再翻牌。""",

            "骑士": """【骑士专属策略】
你的身份是秘密的！不要主动说"我是骑士"。
策略：仔细观察发言找出铁狼，质疑要有把握再行动。""",
...
策略：认真分析发言抿狼坑，不要乱跳身份。""",
            ...
            "狼人": """【狼人专属策略 - 必须遵守】
你必须隐藏自己是狼人的身份！绝对不能说"我是狼"、"我是一匹狼"等话！
策略选择：
1. 悍跳：跳神职（预言家/女巫/猎人）对抗真神职获取信任
2. 潜水：低调发言像好人，减少被注意
3. 倒钩：假装站边真预言家，混淆好人判断
发言要点：像好人一样分析场上局势，绝对不能暴露自己是狼。""",

            "狼王": """【狼王专属策略 - 必须遵守】
你必须隐藏自己是狼人的身份！死亡时可以开枪带走一人（但被投票出局无法发动）。
策略：可以悍跳或潜水，死亡时谨慎选择开枪目标。""",

            "白狼王": """【白狼王专属策略 - 必须遵守】
你必须隐藏自己是狼人的身份！白天可以自爆带走一人。
策略：关键时刻可以自爆带神职，自爆时机要把握好。""",

            "狼美人": """【狼美人专属策略 - 必须遵守】
你必须隐藏自己是狼人的身份！每晚魅惑一名玩家，死亡时带走魅惑目标。
策略：可以悍跳或潜水，利用魅惑做身份。""",

            "恶灵骑士": """【恶灵骑士专属策略 - 必须遵守】
你必须隐藏自己是狼人的身份！预言家查验你会显示为好人。
策略：利用"查验显示好人"的特性强势发言博取信任。""",

            "丘比特": """【丘比特专属策略】
你的身份是秘密的。
策略：第一晚连线情侣，争取情侣存活到最后获胜。""",

            "野孩子": """【野孩子专属策略】
你的身份是秘密的，偶像死亡后你会变成狼人。
策略：第一晚选择重要玩家作为偶像，观察偶像阵营。""",

            "吹笛人": """【吹笛人专属策略】
你的身份是秘密的。
策略：迷惑所有存活玩家即可获胜，从关键玩家开始迷惑。""",

            "狐狸": """【狐狸专属策略】
你的身份是秘密的。
策略：查验3个相邻位置，有狼人则失去技能，无狼人则免疫狼刀。""",

            "熊": """【熊专属策略】
你的身份是秘密的。
策略：相邻位置有狼人时法官会宣布咆哮，利用这个信息抿狼。""",
        }
        return strategies.get(role, """【通用策略】
你的身份是秘密的，不要主动暴露。通过分析发言来判断其他玩家身份。""")

    def get_system_prompt(self) -> str:
        """生成系统提示"""
        from .referee import get_role_description_summary, get_game_config_summary, ROLE_CONFIGS
        ...
        if role:
            role_desc = f"你的角色是【{role}】，这是你的秘密，绝对不要在发言中暴露！"
            role_strategy = self._get_role_strategy(role)

        # 获取本局阵容配置
        config_info = ""
        if self.game_state and self.game_state.players:
            player_count = len(self.game_state.players)
            if player_count in ROLE_CONFIGS:
                config = ROLE_CONFIGS[player_count]
                config_info = get_game_config_summary(player_count, config)
...
【关键规则】身份隐藏：
- 【禁止】发言开头说"我是XX"，除了预言家报查验外
- 【禁止】直接说"我是好人"或"我是狼"
- 狼人必须伪装成好人发言，绝不能说自己是狼
- 好人通过分析他人发言来判断身份，不要主动跳身份
- 只有在被质疑、被投票时才考虑跳身份自证
...
{get_role_description_summary()}
{config_info}

发言时请注意：
- 【重要】不要在发言开头说身份，用分析来代替
- 发言要像你真正身份的人，狼人要模仿好人发言
- 适当使用情绪技巧增加感染力
- 考虑战术配合
- 狼人可以使用悍跳（跳神职）或潜水策略
