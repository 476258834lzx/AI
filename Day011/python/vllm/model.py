...
class StorierAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self._rep = self._config.num_attention_heads // self._config.num_key_value_heads
        ...
        self._ow = ColumnParallelLinear(#行大于列用行并行，列大于行用列并行
    ...
    def forward(self,
        ...
        _attn_output = _attn_output.reshape(
            -1,
            self._config.num_key_value_heads,
            self._rep,
            self._head_size)
        _attn_output = _attn_output.permute(0, 2, 1, 3)
        _attn_output = _attn_output.reshape(-1, self._config.num_key_value_heads*self._rep*self._head_size)
        # _attn_output = _attn_output.reshape(-1, 1536)

        _output, _ = self._ow(_attn_output)
        return _output


class StorierFFN(nn.Module):
...
class StorierLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self._att_layer = StorierAttention(
            config=self._config,
...
class StorierForCausalLM(nn.Module, SupportsLoRA):
    ...
    def __init__(
        ...
        self._model = StorierModel(
            config=self._config,
            cache_config=self._cache_config,
            quant_config=self._quant_config)

        # self._lm_head = VocabParallelEmbedding(30000,1536)#self._model._emb
    ...
    def sample(self,
        ...
        # print(logits[0,1428:1438])
        # print(logits[0,0:10])
        # print(torch.topk(logits,5,dim=-1))
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        for _name, _loaded_weight in weights:
            ...
            if _name.find("_qw") > 0:
                ...
                # _name = _name.replace("_ow", "_down_proj")
...
modeling_auto.MODEL_FOR_CAUSAL_LM_MAPPING_NAMES["storier"] = "StorierForCausalLM"
