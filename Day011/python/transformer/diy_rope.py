...
def precompute_freqs_cis(dim,end,theta=50000.0,device="cpu"):#生成位置向量列表,设置单位角度
    freqs = 1.0/(theta ** (torch.arange(0,dim,2,device=device)[:(dim//2)].float()/dim))#对50000按步长开方的倒数,每个向量两两一组(dim=32,32/2=16)
    ...
    freqs_cis=torch.polar(torch.ones_like(freqs),freqs)#创建模为1的极坐标,横轴为实数1,纵轴为虚部1i
    return freqs_cis
